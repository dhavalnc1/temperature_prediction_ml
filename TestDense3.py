...
# Test model with same input/output size, windows-dense same
...
def create_dataset(dataset, window_size):
    ...
    for i in range(len(dataset) - window_size - window_size - 1):
        a = dataset[i:(i + window_size), 0]
        data_X.append(a)
        b = dataset[(i+window_size):(i + window_size + window_size), 0]
        data_Y.append(b)
    return(np.array(data_X), np.array(data_Y))
...
print("Test 1 shape:", test_X.shape)
print("Test 1 shape[0]:", test_X[0].shape)
temp = model1.predict(test_X)
