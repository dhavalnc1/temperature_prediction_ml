...
# temperature_dataset = temperature_data_raw

# split temperature data into train/test size
# TRAIN_SIZE = 1.0
train_size = int(len(temperature_dataset) - window_size - output_size)
# test_size = len(temperature_dataset) - train_size
train = temperature_dataset[0:train_size, :]
test = temperature_dataset[(len(temperature_dataset) - window_size):len(temperature_dataset), :]
...
def create_test_temperature_dataset(datain, input_size):
    data_X = []
    a = datain[0:input_size, 0]
    data_X.append(a)
...
test_temp_X = create_test_temperature_dataset(test, window_size)


# Reshape the input data into appropriate form for Keras.
train_temp_X = np.reshape(train_temp_X, (train_temp_X.shape[0], 1, train_temp_X.shape[1]))
test_temp_X = np.reshape(test_temp_X, (1, 1, 336))
